...
#define myprint(fmt, args...)                                                \
    do {                                                                                \
        ...
        char buffer[4096];                                                           \
                                                                                             \
        if(stat("/tmp/lym_vap", &statbuff) >= 0)                            \
                filesize = statbuff.st_size;                                          \
        if(filesize >1024*1024)                                                            \
            fp = fopen("/tmp/lym_vap", "w+");                                \
        else                                                                                 \
            fp = fopen("/tmp/lym_vap", "a+");                                  \
        if (fp != NULL) {                                                               \
            memset(buffer, 0, 4096);                                                  \
            sprintf(buffer, "[msg is %s, %s, %d]"fmt, __FILE__, __func__, __LINE__, ##args);   \
        ...
        //#define MYDEBUG
#define myprint1(fmt, args...)                                                           \
            do {                                                                                \
                long file_size = 0;                                                             \
                FILE *fp;                                                                       \
                char buffer[128];                                                              \
                                                                                                \
                fp = fopen("/tmp/lym_vap1", "a+");                                               \
                if (fp != NULL) {                                                               \
                    memset(buffer, 0, 128);                                                    \
                    sprintf(buffer, "[%s, %s, %d]"fmt, __FILE__, __func__, __LINE__, ##args);   \
                    fwrite(buffer, strlen(buffer) , 1, fp);                                     \
                    fwrite("\n", 1, 1, fp);                                                     \
                    fclose(fp);                                                                 \
                }                                                                               \

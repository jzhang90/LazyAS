...
{
	int set_beacon = 0;
       update_station__s del_sta; 

       os_memset(&del_sta , 0x0 ,sizeof(update_station__s));
       os_memcpy(del_sta.sta_mac ,sta->addr,ETH_ALEN);
       os_memcpy(del_sta.ap_mac ,hapd->own_addr,ETH_ALEN);
       os_memcpy(del_sta.ssid , hapd->conf->ssid.ssid , hapd->conf->ssid.ssid_len);
       del_sta.ssid[hapd->conf->ssid.ssid_len] = '\0';
       fnii_send_msg(FNII_MSG_DEL_STA,(void *)&del_sta);

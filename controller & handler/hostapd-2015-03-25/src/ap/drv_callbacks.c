...
static struct hostapd_data * get_hapd_bssid_dense(struct hostapd_iface *iface,
					    const u8 *bssid,const u8 *station)
{
	size_t i;
	if (bssid == NULL)
		return NULL;
	...
	for (i = 0; i < iface->num_bss; i++) {
		if (os_memcmp(bssid, iface->bss[i]->own_addr, ETH_ALEN) == 0)
		{
            if(os_memcmp(station,iface->bss[i]->own_sta,ETH_ALEN) == 0)
			    return iface->bss[i];
            else if(iface->bss[i]->flag_priv == VAP_UNLOCK)
            {
                return iface->bss[i];
            }
            else
		       ...
		       union wpa_event_data *data)
{
	...
	if (event == EVENT_RX_MGMT && data->rx_mgmt.frame &&
		...
		{
                    os_memset(&probe_data,0,sizeof(probe_data));
                    level = MSG_EXCESSIVE;
                    probe_data.channel = data->rx_mgmt.freq;
                    probe_data.rssi = data->rx_mgmt.ssi_signal;
					
                    if(probe_data.rssi > -10)
                        probe_data.rssi = -10;
                    os_memcpy(probe_data.station_mac,hdr->addr2,ETH_ALEN);
					
					
                    if(!IS_LOCAL_MAC(probe_data.station_mac)&&probe_data.rssi > -90)
                    {
                        fnii_send_msg(FNII_MSG_PROBE_INFO,(void *)&probe_data);
                    }
		}
	}

...
extern struct _fnii_dense_station_hash_s g_dense_station_hash;
#define  PROBE_DROP     0
#define  PROBE_PROC     1
		      ...
		      struct hostapd_frame_info *fi)
{
	...
	int noack;
       int broadcast;
       char  proc_flag = PROBE_PROC;
       char iphone_mac[6] = {0xb0,0x9f,0xba,0x41,0x4f,0x13};
       cmd_msg_s addr ;
	...
	struct hostapd_ubus_request req = {
       ...
       os_memcpy(addr.bssid, hapd->own_addr, ETH_ALEN);
       os_memcpy(addr.stamac, mgmt->sa, ETH_ALEN);
       if(broadcast &&
        ((!IS_LOCAL_MAC(mgmt->sa)) || g_dense_station_hash.station_used >= STAION_MAX) 
        && fnii_dense_station_find(&addr) == FNII_RET_ERR)
	{
              proc_flag = PROBE_DROP;
              if(g_dense_station_hash.station_used >= STAION_MAX)
                	return;
	}
...
#define BEACON_TAIL_BUF_SIZE 512
	...
	head->frame_control = IEEE80211_FC(WLAN_FC_TYPE_MGMT,
					   WLAN_FC_STYPE_BEACON);
	head->duration = host_to_le16(0);
	os_memcpy(head->da, statinmac, ETH_ALEN);
...
struct hostapd_data *
ieee802_11_find_bss_odin(struct hostapd_iface *iface,
                                   u8 *bssid,
                                   u8 *stamac)
{
    size_t i;
    ...
    struct sta_info *sta = NULL;
    char  macnull[ETH_ALEN] = {0x66,0x55,0x44,0x33,0x22,0x11};
    if(iface == NULL ||
       bssid == NULL)
    {
        return NULL;
    ...
    if(stamac == NULL)
    {
        ...
        {
            bss = iface->bss[i];
            if(os_memcmp(bssid,bss->own_addr,ETH_ALEN) == 0)
            {
                return bss;
            }
        }
    ...
    else
    {
        ...
        {
            bss = iface->bss[i];
            if (os_memcmp(bss->own_addr,bssid,ETH_ALEN) == 0)
            {
                //myprint("delete vap success\n");
                sta = ap_get_sta(bss, stamac);
                if (sta)
                {
                    hostapd_drv_sta_disassoc(bss,stamac,WLAN_REASON_DISASSOC_AP_BUSY);
                    ap_sta_disassociate(bss, sta, WLAN_REASON_DISASSOC_AP_BUSY);
                }
                //sta->timeout_next = STA_DEAUTH;
                //eloop_cancel_timeout(ap_handle_timer, bss, sta);
		        //eloop_register_timeout(AP_DEAUTH_DELAY, 0, ap_handle_timer,bss, sta);
                os_memcpy(stamac,macnull,ETH_ALEN);
                return bss;
            }
        }
    }
...
int
ieee802_11_update_bssflag_odin(struct hostapd_data *bss,
								   			u8 flag)
{
	if(bss == NULL)
	{
	...
	if(flag == DEL_IF)
	{
		bss->flag_priv = VAP_IDLE;
	}
	else
	{
        if(GET_MAC_IDF(bss->own_sta) == LOCAL_MAC)
        {
            bss->flag_priv = VAP_UNLOCK;
        }
        else
        {
    		bss->flag_priv = VAP_LOCK;
        }
	}
	return  0;
}
...
int ieee802_11_del_station_of_vap(struct hostapd_data *bss,char *addr)
{
    ...
    {
        myprint("del station");
        hostapd_drv_sta_disassoc(bss,addr,WLAN_REASON_DISASSOC_AP_BUSY);
        ap_sta_disassociate(bss, sta, WLAN_REASON_DISASSOC_AP_BUSY);
    }
...
int ieee802_11_del_offline(struct hostapd_data *bss,char *addr)
{
    struct sta_info *sta = NULL;
    if(bss == NULL || addr == NULL)
    {
    ...
    if (sta)
    {
        hostapd_drv_sta_only_disassoc(bss,addr,WLAN_REASON_DISASSOC_AP_BUSY);
    }
...
int ieee802_11_del_station(struct hostapd_iface *iface,struct _fnii_dense_station_s *addr)
{
    ...
    {
        bss = iface->bss[i];
        if(os_memcmp(addr->ap, bss->own_addr , ETH_ALEN) == 0)
        {
            sta = ap_get_sta(bss, addr->station);
            if (sta)
            {
                myprint("del station");
                hostapd_drv_sta_disassoc(bss,addr->station,WLAN_REASON_DISASSOC_AP_BUSY);
                ap_sta_disassociate(bss, sta, WLAN_REASON_DISASSOC_AP_BUSY);
            }
            break;
        }
...
int 
ieee802_11_update_beacons_odin(struct hostapd_iface *iface,cmd_msg_s *ap_info)
{
	struct hostapd_data *bss = NULL;
	struct wpa_driver_ap_params params;
	struct wpabuf *beacon, *proberesp, *assocresp;
	...
	char  macnull[ETH_ALEN] = {0x66,0x55,0x44,0x33,0x22,0x11};
	struct timeval end;
	struct sta_info *sta_onlyfirst = NULL;
	if(iface == NULL ||
	   ap_info == NULL)
	{
		return RET_ERR;
	}
	if(ap_info->type >= TYPE_MAX ||
	   ap_info->type < DEL_IF)
	{
		return RET_TYPE_UNKOWN;
	}
	if(ap_info->type == NEW_IF)
	{
		//myprint("add vap mode\n");
		bss = ieee802_11_find_bss_odin(iface,ap_info->bssid,NULL);
		if(bss != NULL)
		{
		    //myprint("add vap\n");
		    os_memcpy(bss->own_sta,ap_info->stamac, ETH_ALEN);
		}
	}
	else
	{
		//myprint("del vap mode\n");
		bss = ieee802_11_find_bss_odin(iface,ap_info->bssid,ap_info->stamac);
		if(bss != NULL)
		{
		    //myprint("del vap\n");
		    os_memcpy(bss->own_sta,ap_info->stamac, ETH_ALEN);
	...
	bss->beacon_set_done = 1;
	if (ieee802_11_build_ap_params_odin(bss, &params,ap_info->stamac) < 0)
		return RET_ERR;
	if (hostapd_build_ap_extra_ies(bss, &beacon, &proberesp, &assocresp) < 0)
		goto fail;
	params.beacon_ies = beacon;
	params.proberesp_ies = proberesp;
	params.assocresp_ies = assocresp;
	res = hostapd_drv_set_ap(bss, &params);
	hostapd_free_ap_extra_ies(bss, beacon, proberesp, assocresp);
	if (res)
	{
		res = RET_ERR;
		myprint( "Failed to set beacon parameters");
	}
	else
	{
		ret = RET_SUSS;
	}
    ieee802_11_update_bssflag_odin(bss,ap_info->type);
fail:
	ieee802_11_free_ap_params(&params);
	return ret;	
}
...
ieee802_11_set_own_sta_odin(struct hostapd_data * bss)
{
	...
	bss->beacon_set_done = 1;
	if (ieee802_11_build_ap_params_odin(bss, &params,bss->own_sta) < 0)
		return RET_ERR;
	if (hostapd_build_ap_extra_ies(bss, &beacon, &proberesp, &assocresp) < 0)
		goto fail;
	params.beacon_ies = beacon;
	params.proberesp_ies = proberesp;
	params.assocresp_ies = assocresp;
	hostapd_drv_set_ap(bss, &params);
	hostapd_free_ap_extra_ies(bss, beacon, proberesp, assocresp);
    os_memset(&mac_rrpair, 0, sizeof(mac_rrpair));
    os_memcpy(mac_rrpair.sta_mac,bss->own_sta,ETH_ALEN);
    os_memcpy(mac_rrpair.vif_mac,bss->own_addr,ETH_ALEN);
fail:
	ieee802_11_free_ap_params(&params);
    fnii_send_msg(FNII_MSG_UPDATE_STATION,&mac_rrpair);
	return RET_SUSS;	
}
...
ieee802_11_set_beacon_odin(struct hostapd_data * bss,char flag)
{
    ...
    struct wpabuf *beacon, *proberesp, *assocresp;
    int res, ret = RET_ERR;
    char  stamac[ETH_ALEN] = {0x66,0x55,0x44,0x33,0x22,0x11};
    os_memcpy(bss->own_sta,stamac, ETH_ALEN);
    bss->beacon_set_done = 1;
    if(flag == 1)
        os_memset(stamac, 0xff, ETH_ALEN);
    if (ieee802_11_build_ap_params_odin(bss, &params,stamac) < 0)
        return RET_ERR;
    if (hostapd_build_ap_extra_ies(bss, &beacon, &proberesp, &assocresp) < 0)
        goto fail;
    params.beacon_ies = beacon;
...
ieee802_11_update_all_beacons_odin(struct hostapd_iface *iface,char flag)
{
	...
	{
		bss = iface->bss[i];
              ieee802_11_set_beacon_odin(bss,flag);
    }
}

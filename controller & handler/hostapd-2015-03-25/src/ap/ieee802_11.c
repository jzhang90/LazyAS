			...
			struct hostapd_frame_info *fi)
{
 ...
 fail:
    ...
    /*
    if(os_memcmp(hapd->own_sta,mgmt->sa,ETH_ALEN) != 0
        && hapd->flag_priv == VAP_UNLOCK)
    {
        os_memcpy(hapd->own_sta,mgmt->sa,ETH_ALEN);
        ieee802_11_set_own_sta_odin(hapd);
    }
    hapd->flag_priv = VAP_LOCK;
    */
}

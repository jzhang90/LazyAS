			     ...
			     const u8 *addr, int reason)
{
	if (hapd->driver == NULL || hapd->driver->sta_disassoc == NULL)
		return 0;
       myprint("hostapd_drv_sta_disassoc");
...
int hostapd_drv_sta_only_disassoc(struct hostapd_data *hapd,
			     const u8 *addr, int reason)
{
	if (hapd->driver == NULL || hapd->driver->sta_disassoc == NULL)
		return 0;
	return hapd->driver->send2sta_disassoc(hapd->drv_priv, hapd->own_addr, addr,
					  reason);
}

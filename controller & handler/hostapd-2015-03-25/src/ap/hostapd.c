...
int hostapd_setup_interface_complete(struct hostapd_iface *iface, int err)
{
    ...
    pthread_t statistic_ID;
    pthread_t flow_id_thread = 0;
...
dfs_offload:
    ...
    myprint("start  click thread");
    pthread_create(&statistic_ID,NULL,
                       fnii_receive_msg_thread,
                       (void *)iface);
    pthread_create(&flow_id_thread,NULL,
                       fnii_statistics_flow_thread,(void *)iface);
...
int hostapd_setup_interface(struct hostapd_iface *iface)
{
    ...
    {
        hapd = iface->bss[0];
        os_memcpy(hapd->own_sta,stamac,ETH_ALEN);

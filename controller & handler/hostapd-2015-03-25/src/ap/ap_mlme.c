				    ...
				    struct sta_info *sta, u16 reason_code)
{
	...
	fnii_del_sta(sta->addr,sta->ssid->ssid,hapd->conf->iface,hapd->iconf->channel);
...
{
    update_station__s  new_sta;
    cmd_msg_s  addr;
    hostapd_logger(hapd, sta->addr, HOSTAPD_MODULE_MLME,
		       HOSTAPD_LEVEL_DEBUG,
		       "MLME-ASSOCIATE.indication(" MACSTR ")",
		       MAC2STR(sta->addr));
    os_memset(&new_sta , 0x0 ,sizeof(update_station__s));
    os_memcpy(new_sta.sta_mac ,sta->addr,ETH_ALEN);
    os_memcpy(addr.stamac ,sta->addr,ETH_ALEN);
    os_memcpy(addr.bssid,hapd->own_addr,ETH_ALEN);
    os_memcpy(new_sta.ap_mac ,hapd->own_addr,ETH_ALEN);
    os_memcpy(new_sta.ssid , hapd->conf->ssid.ssid , hapd->conf->ssid.ssid_len);
    new_sta.ssid[hapd->conf->ssid.ssid_len] = '\0';
    
    fnii_send_msg(FNII_MSG_NEW_STA,(void *)&new_sta);
    if(fnii_dense_station_add(&addr) == FNII_RET_OK)
    {
        myprint("Forced on-line "MACSTR"",MAC2STR(sta->addr));
    }
    else
    {
        myprint("General on-line "MACSTR"",MAC2STR(sta->addr));
    }
	fnii_new_sta(sta->addr,sta->ssid->ssid,hapd->conf->iface,hapd->iconf->channel);

...
void ht_insert_he(hash_table *table, hash_entry *entry){
    ...
    {
    	//printf("1\n");
    	if(t.tv_sec - ((struct timeval*)tmp->value)->tv_sec > AGE_TIMEOUT)
    	{//timeout session
    		table->key_count--;
			if(prev == NULL)
			{
    ...
    //printf("5\n");
        ...
        he_set_value(table->flags, find_ret, entry->value, entry->value_size);
		//printf("51\n");
        he_destroy(table->flags, entry);
		//printf("52\n");
